...
| Deposit watcher      | Read USDC `Transfer` logs from the native view (`0xffff…fffe`), scaled down by 10^12: a wallet's value send writes a log only there, an ERC-20 transfer writes one on both views. Balance reconciliation stays on `balanceOf`. Do not route Arc into the native ETH block scanner. |

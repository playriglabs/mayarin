...
/** A transaction with locked amounts, in USDC (6 dp): 5.000000 in, 0.025000 fee, 4.975000 net. */
function pricedTransaction(): ClearingTransaction {
  return {
    ...
    netAmount: money(4_975_000n, "USDC"),
    createdAt: NOW,
    updatedAt: NOW,
    version: 5,
  };
...
describe("internalSettledPosting", () => {
  test("debits settlement in flight and credits merchant holding for the net", () => {
    ...
    expect(posting.entries).toEqual([
      {
        accountCode: "SETTLEMENT_IN_FLIGHT:USDC",
        direction: "DEBIT",
        amount: money(4_975_000n, "USDC"),
      },
      {
        accountCode: "MERCHANT_HOLDING:USDC",
        direction: "CREDIT",
        amount: money(4_975_000n, "USDC"),
      },
    ]);
...
describe("swapPosting", () => {
  ...
  test("refuses an output denominated in something other than the settlement asset", () => {
    expect(() => swapPosting(depositTransaction(), money(3_000_000n, "USDT"))).toThrow(

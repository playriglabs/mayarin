...
describe("deposit leg", () => {
  ...
  test("an intent with a rail locks a deposit amount and an address", async () => {
    const harness = createHarness({
      ...
      // 50,000.00 IDR at 320 minor USDT units per whole IDR is 16.000000 USDT.
      rates: { "IDR/USDC": 100n, "IDR/USDT": 320n },
    });
    const intent = await harness.confirmedIntent({
    ...
    expect(transaction.deposit?.asset).toBe("USDT");
  ...
  test("the settlement leg is unaffected by the deposit leg", async () => {
    ...
    // 50,000.00 IDR at 100 minor USDC units per rupiah is 5.000000 USDC, 0.50% fee.
